...
export function Toast({ type, children }: IToastProps) {
  ...
  return (
    <div
      className={`fixed top-3 mx-auto text-lg font-normal px-4 py-2 rounded-xl z-100 ${bgColor}`}
    >
      {children}
    </div>
  );
...
export function ToastMatcherFailure() {
  return (
    <Toast type="failure">
      ❌ Failed to get match results – try again in a while
    </Toast>
  );
}
